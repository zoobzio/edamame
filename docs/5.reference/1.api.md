...
Returns a cereal Compound builder for set operations (UNION, INTERSECT, EXCEPT). Unlike other operations, compound queries are constructed directly from a spec rather than registered as capabilities.
...
#### PrepareQuery / PrepareSelect / PrepareUpdate / PrepareDelete / PrepareAggregate
...
Prepares a capability's SQL statement for repeated execution. Returns a prepared statement that can be executed multiple times with different parameters.
